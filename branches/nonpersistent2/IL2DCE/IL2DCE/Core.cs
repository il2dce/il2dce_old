...
namespace IL2DCE
{
    public class Core : ICore
    {
        ...
        private List<Depot> redDepots = new List<Depot>();
        private List<Depot> blueDepots = new List<Depot>();
        private List<Airbase> redAirbases = new List<Airbase>();
        private List<Airbase> blueAirbases = new List<Airbase>();
                    ...
                    {
                        {
                            ...
                            int fromhere = 1;
                            string CampInfoFileName = "CampaignInfo" + this.tempphase + ".ini";
                            if (campaignFolder.GetFiles(CampInfoFileName) != null && campaignFolder.GetFiles(CampInfoFileName).Length == 1)
                            {
                                ISectionFile campaignInfoFile = game.gameInterface.SectionFileLoad(campaignsFolderPath + "/" + campaignFolder.Name + "/" + CampInfoFileName);
            ...
            if (confFile.exist("Main", "campaignsFolder"))
            {
                ...
                if (campaignsFolder.Exists && campaignsFolder.GetDirectories() != null && campaignsFolder.GetDirectories().Length > 0)
                {
                    ...
                    foreach (System.IO.DirectoryInfo campaignFolder in campaignsFolder.GetDirectories())
                    {
                        ...
                        if (campaignFolder.GetFiles(CampInfoFileName) != null && campaignFolder.GetFiles(CampInfoFileName).Length == 1)
                        {
                            ISectionFile campaignInfoFile = GamePlay.gpLoadSectionFile(campaignsFolderPath + "/" + campaignFolder.Name + "/" + CampInfoFileName);
        ...
        {
            if (!Career.Date.HasValue)      // Starting a new career
            {
                //Career.Phase = 0;
            ...
            {
                Career.Date = Career.Date.Value.Add(new TimeSpan(1, 0, 0, 0));  // Continue career

                if (game is IGameSingle)
                {
                    if ((game as IGameSingle).BattleSuccess == EBattleResult.SUCCESS)
                    {
                        string df = Career.Difficulty.ToString();
            ...
            // this is where we could check for new campaign phase (by date for now)
            // is the campaign or phase over??? - vP

            // OMG getting the dates is tough.... - vP
            string dte = Career.Date.ToString();
            string end = Career.CampaignInfo.EndDate.ToString();
            string[] DDate = dte.Split('/');
            string[] EEnd = end.Split('/');
            string[] xdate = DDate[2].Split(' ');
            string[] xend = EEnd[2].Split(' ');
            string testt = xdate[0] + DDate[1] + DDate[0];
            string endd = xend[0] + EEnd[1] + EEnd[0];
            int d_te = Convert.ToInt32(testt);
            int e_nd = Convert.ToInt32(endd);
            int getdateshere = 1;
            if (e_nd < d_te)
            {
                Career.Phase += 1;

                // possibly move this bracked below following stuff
                //   edit campaign variables here to make it load next phase properly
                int thisiswhereto_changecampaigninfo = 0;
      ...
      //      ISectionFile campaignInfoFile = game.gameInterface.SectionFileLoad(campaignsFolderPath + "/" + campaignFolder.Name + "/" + CampInfoFileName);
        ...
        public void InitCampaign()
        {
            ...
            for (int i = 0; i < templateFile.lines("Stationary"); i++)
            {
                ...
                if (valueParts.Length > 4)
                {
                    ...
                    if (valueParts[0] == "Stationary.Bf-109E-1")
                    {
                        double x;
                        double y;
                        double.TryParse(valueParts[2], System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture.NumberFormat, out x);
                        double.TryParse(valueParts[3], System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture.NumberFormat, out y);
                        Airbase ac = new Airbase(key, x, y);
                        blueAirbases.Add(ac);
                    }
                    if (valueParts[0] == "Stationary.HurricaneMkI_dH5-20")
                    {
                        double x;
                        double y;
                        double.TryParse(valueParts[2], System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture.NumberFormat, out x);
                    ...
                    if (valueParts[0] == "Stationary.Environment.FuelDrum_GER1_1")
                    {
                        double x;
                        double y;
                        double.TryParse(valueParts[2], System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture.NumberFormat, out x);
                        double.TryParse(valueParts[3], System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture.NumberFormat, out y);
                        Depot fuel = new Depot(key, x, y);
                        blueDepots.Add(fuel);
                    }
                    if (valueParts[0] == "Stationary.Environment.FuelDrum_UK1_1")
                    {
                        double x;
                        double y;
                        double.TryParse(valueParts[2], System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture.NumberFormat, out x);
        ...
        private List<Depot> getFriendlyFuels(int armyIndex)
        {
            if (armyIndex == 1)
            {
                return redDepots;
        ...
        private List<Depot> getEnemyFuels(int armyIndex)
        {
            if (armyIndex == 1)
            {
                return blueDepots;
        ...
        private List<Airbase> getFriendlyAcs(int armyIndex)
        {
            if (armyIndex == 1)
            {
                return redAirbases;
            ...
            {
                return new List<Airbase>();
            }
        }

        private List<Airbase> getEnemyAcs(int armyIndex)
        {
            if (armyIndex == 1)
            {
                return blueAirbases;
        ...
        private bool isMissionTypeEscorted(EMissionType missionType)
        {
            if (missionType == EMissionType.ATTACK_ARMOR
                ...
                || missionType == EMissionType.ATTACK_AIRBASE
        ...
        private bool isMissionTypeAvailable(AirGroup airGroup, EMissionType missionType)
        {
            ...
            {
                List<GroundGroup> groundGroups = getAvailableEnemyGroundGroups(airGroup.ArmyIndex, new List<EGroundGroupType> { EGroundGroupType.Armor, EGroundGroupType.Train, EGroundGroupType.Vehicle });
                if (groundGroups.Count > 0)
                {
                    return true;
                }
            ...
            {
                List<Depot> fuels = getEnemyFuels(airGroup.ArmyIndex);
                if (fuels.Count > 0)
                {
                    return true;
                }
            ...
            else if (missionType == EMissionType.ATTACK_AIRBASE)
            {
                List<Airbase> acs = getEnemyAcs(airGroup.ArmyIndex);
                if (acs.Count > 0)
                {
                    return true;
                }
            ...
            {
                List<GroundGroup> groundGroups = getAvailableEnemyGroundGroups(airGroup.ArmyIndex, new List<EGroundGroupType> { EGroundGroupType.Vehicle, EGroundGroupType.Train });
                if (groundGroups.Count > 0)
                {
                    return true;
                }
            ...
            {
                List<GroundGroup> groundGroups = getAvailableEnemyGroundGroups(airGroup.ArmyIndex, new List<EGroundGroupType> { EGroundGroupType.Armor, EGroundGroupType.Vehicle, EGroundGroupType.Train });
                if (groundGroups.Count > 0)
                {
                    return true;
                }
        ...
        private void createAirOperation(ISectionFile sectionFile, IBriefingFile briefingFile, AirGroup airGroup, EMissionType missionType, bool allowIntercept, AirGroup forcedEscortAirGroup)
        {
            if (isMissionTypeAvailable(airGroup, missionType))
            {
                ...
                {
                    GroundGroup groundGroup = getAvailableRandomEnemyGroundGroup(airGroup.ArmyIndex, new List<EGroundGroupType> { EGroundGroupType.Armor, EGroundGroupType.Vehicle, EGroundGroupType.Train });
                    createRandomGroundOperation(sectionFile, groundGroup);
                    double altitude = getRandomAltitude(randomAircraftParametersInfo);
                ...
                else if (missionType == EMissionType.ATTACK_RADAR)
                {
                    ...
                    if (radars.Count > 0)
                    {
                        ...
                        airGroup.GroundAttack(missionType, radar, altitude, escortAirGroup);    // uses attack target
                    }
                }
                else if (missionType == EMissionType.ATTACK_DEPOT)
                {
                    ...
                    if (fuels.Count > 0)
                    {
                        int fuelIndex = rand.Next(fuels.Count);
                        Depot fuel = fuels[fuelIndex];
                        double altitude = getRandomAltitude(randomAircraftParametersInfo);

                        airGroup.GroundAttack(altitude, missionType, fuel, escortAirGroup);   // uses attack area
                    }
                }
                else if (missionType == EMissionType.ATTACK_AIRBASE)
                {
                    ...
                    if (acs.Count > 0)
                    {
                        int acIndex = rand.Next(acs.Count);
                        Airbase ac = acs[acIndex];
                        double altitude = getRandomAltitude(randomAircraftParametersInfo);

                        airGroup.GroundAttack(missionType, altitude, ac, escortAirGroup);   // uses attack area
                    }
                }
                else if (missionType == EMissionType.ATTACK_SHIP)
                {
                ...
                {
                    GroundGroup groundGroup = getAvailableRandomEnemyGroundGroup(airGroup.ArmyIndex, new List<EGroundGroupType> { EGroundGroupType.Vehicle, EGroundGroupType.Train });
                    createRandomGroundOperation(sectionFile, groundGroup);
                    double altitude = getRandomAltitude(randomAircraftParametersInfo);
